...
import {
  ...
  Building2, Phone, Mail, Shield, Eye, EyeOff
} from 'lucide-react';
...
export default function TeachersPage() {
  ...
  const designationColors = {
    ...
    'Associate Professor': 'bg-blue-100 dark:bg-blue-900/30 text-blue-700 dark:text-blue-300',
    'Assistant Professor': 'bg-emerald-100 dark:bg-emerald-900/30 text-emerald-700 dark:text-emerald-300',
    'Lecturer': 'bg-purple-100 dark:bg-purple-900/30 text-purple-700 dark:text-purple-300',
  };
    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-slate-500 dark:text-slate-400 mt-1">Manage faculty members & credentials</p>
        </div>
      ...
      {loading ? (
        ...
        <div className="text-center py-16 text-slate-400"><GraduationCap size={48} className="mx-auto mb-3 opacity-40" /><p className="font-semibold">No teachers found</p></div>
      ) : (
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="bg-slate-50 dark:bg-slate-800/50">
                  ...
                  <th className="text-left px-5 py-3 font-semibold text-slate-600 dark:text-slate-400">Teacher ID</th>
                  <th className="text-left px-5 py-3 font-semibold text-slate-600 dark:text-slate-400">Department</th>
                  <th className="text-left px-5 py-3 font-semibold text-slate-600 dark:text-slate-400">Designation</th>
              ...
              <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
                {filtered.map((t, i) => (
                  <motion.tr key={t._id} initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: i * 0.03 }}
                    ...
                    <td className="px-5 py-3 font-mono text-xs text-slate-600 dark:text-slate-400">{t.teacherId}</td>
                    <td className="px-5 py-3 text-slate-600 dark:text-slate-400">{t.department || '—'}</td>
                    <td className="px-5 py-3">
                      {t.designation ? (
                        <span className={`px-2 py-0.5 rounded-full text-xs font-bold ${designationColors[t.designation] || 'bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-400'}`}>{t.designation}</span>
                      ) : '—'}
                    </td>
                    <td className="px-5 py-3 text-slate-500 dark:text-slate-400">{t.contactNo || '—'}</td>
                    <td className="px-5 py-3 text-right">
                      <div className="flex items-center justify-end gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                        <button onClick={() => openEdit(t)} className="p-1.5 rounded-lg hover:bg-slate-100 dark:hover:bg-slate-800 text-slate-400 hover:text-indigo-500 transition-colors"><Pencil size={15} /></button>
                        <button onClick={() => setDeleteId(t._id)} className="p-1.5 rounded-lg hover:bg-slate-100 dark:hover:bg-slate-800 text-slate-400 hover:text-rose-500 transition-colors"><Trash2 size={15} /></button>
                      </div>
                    </td>
                  </motion.tr>
                ))}
              ...
              <form onSubmit={handleSave} className="p-6 space-y-4">
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div>
                    ...
                    <input required value={form.teacherId} onChange={e => setForm({ ...form, teacherId: e.target.value })}
                      className="w-full px-3 py-2 rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 text-sm text-slate-800 dark:text-white focus:ring-2 focus:ring-indigo-500 outline-none" />
                  </div>
                </div>
                <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <input value={form.contactNo} onChange={e => setForm({ ...form, contactNo: e.target.value })} placeholder="+880..."
                    className="w-full px-3 py-2 rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 text-sm text-slate-800 dark:text-white focus:ring-2 focus:ring-indigo-500 outline-none" />
                </div>
                <div>
