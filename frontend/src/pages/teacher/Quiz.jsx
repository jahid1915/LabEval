...
import { Save, Award, ArrowLeft, FileSpreadsheet } from 'lucide-react';
...
const Quiz = () => {
  ...
  const fetchData = useCallback(async () => {
    ...
    setLoading(true);
    try {
      const [stuRes, quizRes, configRes] = await Promise.all([
        api.get(`/teacher/students/any?department=${department}&series=${series}`),
        api.get(`/teacher/records/quiz/${courseId}`),
  ...
  const handleMarkChange = (studentId, value) => {
    ...
    else if (numVal > config.quiz) numVal = config.quiz;
  ...
  const getStudentMark = (studentId) => {
    const record = quizRecords.find(r => (r.student?._id || r.student) === studentId);
  ...
  const saveAll = async () => {
    ...
    try {
      ...
      toast.success('Quiz marks saved successfully!');
    } catch {
      toast.error('Failed to save quiz marks');
    } finally {
      setSaving(false);
    }
  };
  ...
  const handleBulkImport = () => {
    ...
    // Expecting rows format: rollNumber[tab/comma/space]marks
    ...
    rows.forEach(row => {
      ...
      if (parts.length >= 2) {
        ...
        if (roll && !isNaN(markVal)) {
          ...
          if (student) {
             const marks = Math.min(config.quiz, Math.max(0, markVal));
    ...
    setBulkInput('');
    toast.success(`Loaded ${importedCount} quiz marks from input! Remember to click Save All to commit.`);
  };
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-8">
        <div>
          ...
          <div className="flex items-center gap-3">
            <h1 className="text-3xl font-heading font-extrabold text-slate-800 dark:text-white">Lab Quiz Marks</h1>
            <span className="px-3 py-1 bg-primary/10 text-primary font-bold rounded-lg text-sm">{courseId}</span>
          </div>
          <p className="text-slate-500 mt-2">Manage and record Continuous Lab Quiz evaluations (0-{config.quiz} marks)</p>
        </div>
        ...
        <div className="flex flex-wrap items-center gap-4 bg-white dark:bg-slate-800 p-4 rounded-2xl shadow-sm border border-slate-100 dark:border-slate-700">
          <div>
            <label className="block text-xs font-bold text-slate-400 mb-1">QUIZ DATE</label>
            <input 
              type="date"
              value={currentDate}
      ...
      <div className="bg-white dark:bg-slate-800 rounded-3xl border border-slate-100 dark:border-slate-700 shadow-sm overflow-hidden">
        ...
        <div className="p-6 bg-slate-50/50 dark:bg-slate-800/50 border-b border-slate-100 dark:border-slate-700 flex flex-col sm:flex-row items-center justify-between gap-4">
          <div className="flex items-center gap-3">
            ...
            <h3 className="font-heading font-bold text-lg text-slate-700 dark:text-white">Continuous Quiz Marks Sheet ({rollGroup})</h3>
          </div>
        ...
        {loading ? (
          ...
          <div className="p-16 text-center text-slate-500">
            <Award className="mx-auto w-12 h-12 text-slate-300 mb-4" />
            ...
            <table className="table table-zebra w-full text-slate-700 dark:text-slate-300">
              <thead>
                <tr className="bg-slate-100/50 dark:bg-slate-700 text-slate-600 dark:text-slate-200">
                  ...
                  <th className="font-bold text-center">Quiz Marks (Max {config.quiz})</th>
                </tr>
              </thead>
              <tbody>
        ...
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white dark:bg-slate-800 rounded-3xl p-8 max-w-lg w-full shadow-2xl border border-slate-100 dark:border-slate-700">
            ...
            <p className="text-slate-500 text-sm mb-6">
              Copy rows from Excel containing the **Roll Number** and **Marks**, then paste them below. Formats supported: tab-separated, comma-separated, space-separated.
              <br/><br/>
              **Example:**
